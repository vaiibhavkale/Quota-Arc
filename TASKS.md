...
- [x] `Sources/App/QuotaArcMain.swift` + `AppDelegate` — launches with no window
...
Quota Arc runs **no OAuth flow of its own**. It borrows a credential each tool
...
nothing to adjust: Quota Arc runs no OAuth flow of its own, so it has no session
...
- [x] `WebSessionProvider.signOut()` clears its own cookies — the one true
      logout in the app, because that session is the only one Quota Arc created.
...
- [ ] **Store notary credentials** — needs an app-specific password, so it has
      ...
      `xcrun notarytool store-credentials UsageNotch --apple-id <id> --team-id 6WFPL8B9FB --password <app-specific-password>`
...
- [x] **Reset Quota Arc…** in Settings: clears the defaults domain, caches,
...
Quota Arc cannot switch the account. The credential belongs to Claude Code,
...
- [x] Reset is alone in its own section, and last. It used to sit a few pixels
      ...
      everything.
- [x] The "Quota Arc never signs in" explanation moved into Integrations,
...
## Renaming to Quota Arc

- [x] Every occurrence renamed, including the bundle identifier
      (`com.vaibhavkale.quotaarc`), scheme, target, test target and log subsystem.
...
- [x] The Sparkle feed points at `https://hivinz.com/appcast.xml`.
...
## Decisions needed
- [x] Final app name is Quota Arc (Windows and Mac)
