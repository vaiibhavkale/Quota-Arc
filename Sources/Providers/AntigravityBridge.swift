...
/// because the API judges *which client* is asking and Quota Arc cannot
...
enum AntigravityBridge {
    ...
    static func headlineID(in windows: [LimitWindow]) -> String? {
        let best = windows
            .compactMap { window -> (LimitWindow, Double)? in
                guard let frac = window.usedFraction else { return nil }
                return (window, frac)
            }
            .max(by: { $0.1 < $1.1 })
        return best?.0.id ?? windows.first?.id
    }
