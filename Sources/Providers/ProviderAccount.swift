...
///
/// Worth showing plainly, because Quota Arc never signs in — it borrows a
...
///
/// Quota Arc cannot sign anyone in — it reads a credential the owning tool
...
enum SignInRoute: Equatable {
    ...
    /// The only case where Quota Arc genuinely signs anyone in or out.
    ...
    var signOutCaveat: String {
        switch self {
        case .modal(let name):
            return "Signs out of \(name) — the session belongs to Quota Arc."
