...
final class AntigravityBridgeTests: XCTestCase {
    ...
    /// Gemini at 0% must not hide Claude/GPT usage on the ring.
    func testTheRingFollowsThePoolThatIsActuallyUsed() {
        let windows = [
            LimitWindow(id: "gemini-5h", label: "Gemini Models", usedFraction: 0),
            LimitWindow(id: "3p-5h", label: "Claude and GPT models", usedFraction: 0),
            LimitWindow(id: "gemini-weekly", label: "Gemini Models", usedFraction: 0),
            LimitWindow(id: "3p-weekly", label: "Claude and GPT models", usedFraction: 0.29),
        ]
        XCTAssertEqual(AntigravityBridge.headlineID(in: windows), "3p-weekly")
    }

    func testTheRingStaysOnTheHigherPoolWhenGeminiIsAlsoUsed() {
        let windows = [
            LimitWindow(id: "gemini-5h", label: "Gemini Models", usedFraction: 0.40),
            LimitWindow(id: "3p-weekly", label: "Claude and GPT models", usedFraction: 0.29),
        ]
        XCTAssertEqual(AntigravityBridge.headlineID(in: windows), "gemini-5h")
    }
